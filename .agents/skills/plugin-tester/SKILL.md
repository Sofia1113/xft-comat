...
name: plugin-tester
version: 0.3.0
description: 端到端测试 xft-comat 工作流插件（/xft-comat:pilot + workflowctl.ts + specialist agents）。在 tmux 里启动隔离的 Codex 子会话、通过 --plugin-dir 加载当前 xft-comat 插件、用预置 prompt 覆盖 feature-simple / feature-medium / feature-hard / bugfix / refactor 五种工作流，实时监控会话、模拟澄清和设计决策、检查 .xft-comat 是否只由 workflowctl.ts 维护、判断路由、TDD、agent 分派、skill 记录和验证闭环是否合理。用户说“测试 xft-comat 插件”“跑工作流插件 E2E”“验证 xft-comat workflow”“重跑插件回归”“验证所有 workflow/mode”时使用。
---
...
3. `skills/pilot/scripts/workflowctl.ts` — `.xft-comat` 的唯一写入口和命令契约。
...
- “改了 `pilot`（主 skill）/ `workflowctl.ts` / agents 之后跑一遍看看”。
...
| `.Codex/skills/plugin-tester/scripts/obt-*` | 测试脚本，名称沿用 `obt-*` 以兼容旧调用 |
| `.Codex/skills/plugin-tester/references/` | 测试 prompt 清单和 tmux 交互协议 |
...
.Codex/skills/plugin-tester/scripts/obt-start "<prompt>"
```
...
当前目录没有 `.xft-comat` 任务时，workflow skill 应先做公共前置阶段：接收任务、轻量探索、用 `AskUserQuestion` 或等价提问工具做最小必要澄清。澄清完成后才应执行：

```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/pilot/scripts/workflowctl.ts" route --task "<澄清后的任务摘要>"
```

读取路由框架后，由模型自行判断 mode，再调用：

```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/pilot/scripts/workflowctl.ts" init --topic "<topic>" --mode <mode> --summary "<摘要>"
```
...
- 主会话只做编排、澄清、路由判断、agent 分派、证据核验和最终汇报。
- 主会话不得直接修改任何项目文件、代码文件、测试文件、配置文件或 `.xft-comat` 文档。
- 主会话唯一允许产生文件变更的动作，是调用插件内 `workflowctl.ts` 维护 `.xft-comat/`。
...
报告 dump 路径、命中的反模式、相关 scrollback、疑似根因。若用户要求修复插件，则改 `skills/pilot/`、`agents/` 或 `workflowctl.ts`，再 `obt-reset` 并用同一 prompt 重跑。
...
通路问题先修 `.Codex/skills/plugin-tester/scripts/`；行为问题再修 `xft-comat` 插件本体。
