...
name: plugin-tester
version: 0.3.0
description: 端到端测试 xft-comat 工作流插件（/pilot command + workflowctl.ts + specialist agents/skills）。在 tmux 里启动隔离的 Codex 子会话、通过 --plugin-dir 加载当前 xft-comat 插件、用预置 prompt 覆盖 feature-simple / feature-medium / feature-hard / bugfix / refactor 五种工作流，实时监控会话、模拟澄清和设计决策、检查 .xft-comat 是否只由 workflowctl.ts 维护、判断路由、TDD、agent/skill 分派和验证闭环是否合理。用户说“测试 xft-comat 插件”“跑工作流插件 E2E”“验证 xft-comat workflow”“重跑插件回归”“验证所有 workflow/mode”时使用。
---
...
| `obt-start "<prompt>"` | 起 tmux 会话、`--plugin-dir` 加载插件、发送 `${XFT_COMAT_ENTRYPOINT:-/pilot} <prompt>` |
...
当前目录没有 `.xft-comat` 任务时，pilot command 应先做公共前置阶段：接收任务、轻量探索、用 `requirements-clarifier` skill 做最小必要澄清。澄清完成后才应执行：
...
注意：pilot 主会话不应自行枚举这些阶段——它只循环 `next --task-dir <dir>` 拿“分派包”（stage、dispatch.agents、skill_paths、inputs、outputs_expected、quality_gate、advance_to），分派 agent 后 `advance`。被测会话若手写阶段计划而不调用 `next`，是反模式。
...
- 用 `submit`（写文档 + 登记参与合一，agent 优先）或 `set-doc` 写当前阶段文档。
- review 阶段必须用 `record-review --blocking true|false` 落库审查结论。
- review/fix 阶段必须写入 review 专用文档，不能覆盖 `02-design.md` / `02-design-note.md`；close 后设计文档仍应保留 architect 的方案内容。
- plan 阶段从 `next` 输出的 `available_skills` 筛选 skill，再用 `record-skill` 记录 `required` / `accepted` / `declined` / `skipped`。
- 用 `new-test-round`（首轮由实现 agent 懒创建）和 `check-test` 维护测试轮次和用例状态。
- 结构化 implementation task 必须由 owner 用 `set-task --status done --evidence ...` 闭环；`todo` / `doing` / `blocked` 不能通过 `validate` / `close`。

工作流主会话的强制纪律：

- 主会话只做编排、澄清、路由判断、agent 分派、用户拍板交互、证据核验和最终汇报。
- 主会话不得直接修改任何项目文件、代码文件、测试文件、配置文件或 `.xft-comat` 文档。
- 主会话经 `workflowctl.ts` 写入的文档只有：路由引导的 `00-routing.md` / `01-requirements.md`，以及 decide 阶段的 `record-decision`；其余阶段文档与证据（`submit`/`set-doc`/`record-agent`/`record-skill`/`record-review`/`check-test`）必须由被分派的 agent 自己调用 `workflowctl.ts` 写回（agent 自录）。
- 业务代码和测试变更必须由对应 specialist agent 或明确的外部 skill 执行。
...
- 需求澄清应逐轮追问，每轮 1-3 个真正阻塞落地的问题，并优先使用 `AskUserQuestion` 或当前运行时的等价提问工具。
- `feature-simple` 可轻量合并需求、设计、测试计划。
- `feature-medium` 只有存在真实架构取舍时才要求用户确认；否则记录单一推荐方案。
- `feature-hard` 必须保留 architect 方案和用户关键抉择；plan 阶段 architect 列决策点，decide 阶段主会话用 AskUserQuestion 拿拍板并经 `record-decision` 写入 `02-design.md` 开头。
- `bugfix` 先复现和根因，不走新功能架构设计。
- `refactor` 先行为基线和安全网，不改变外部行为。

### 4. agent 与 skill 使用

被测会话应优先最小分派：简单判断由主 Codex 完成，只有当前阶段有独立判断价值才派 specialist。

合法 agent 名称（指挥/编排由 pilot 主会话承担，无 conductor；skill 筛选内联在 plan 分派包，无 skill-scout）：
...
- 主会话直接修改业务代码、测试、配置或任意项目文件，而不是把实现交给 specialist agent 或外部 skill。
...
- 被分派的 agent 没有自录（不跑 `set-doc`/`record-agent`/`check-test`），而由主会话代写阶段文档或证据；或 agent 绕过 `workflowctl.ts` 直接写 `.xft-comat`。
...
| tail 持续变化且含 `esc to interrupt` | Codex 仍在执行，继续监听 |
...
- 澄清轮次和是否只问阻塞问题。
- workflowctl 命令链是否完整：route / init / advance / next / set-doc / skills list / record-skill / record-agent / check-test。
...
| Codex 起不来或卡 trust | `obt-start` 已用 bypass permissions；仍卡则检查 sandbox 的 Codex 项目记录 |
